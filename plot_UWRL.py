...
df_gt = pd.read_csv("LR_WaterLab_AA_SourceID_1_QC_0_Year_2024.csv", sep=',', comment='#', na_values=-9999)
df_gt.columns = df_gt.columns.str.strip()  # Remove whitespace from column names

# Load ROI data
df_roi = pd.read_csv("water_pixels_UWRL.csv", sep=',')
df_roi['Timestamp'] = pd.to_datetime(df_roi['Timestamp'])
df_roi.set_index('Timestamp', inplace=True)
df_roi = df_roi[~(df_roi == 0).any(axis=1)]
resampled_df_roi = df_roi.resample('15T').mean()
df_roi_reset = resampled_df_roi.reset_index()
...
columns_to_remove = ['UTCOffset', 'DateTimeUTC', 'BGA', 'BattVolt',
       ...
       'ODO_Local', 'ODO_Sat', 'RH_enc', 'Rain_Tot', 'SpCond','StageNaNCounter', 
        ...
        'fDOM', 'pH']
df_gt_filtered.drop(columns_to_remove, axis=1, inplace=True)

# Merge the ground truth and ROI data on the datetime, using nearest available match
merged_df = pd.merge_asof(df_gt_filtered.sort_values('LocalDateTime'),
...
lns1 = ax1.plot(merged_df['Timestamp'], merged_df['Smoothed_Stage'], color=color, linestyle='-.', label='Groundtruth Value')
...
lns3 = ax2.plot(merged_df['Timestamp'], merged_df['Smoothed_Right_Bank_1'], color=color2, label='Smoothed Stream Bank')
...
fig.tight_layout()
fig.savefig('banksVSgt_UWRL.png', dpi=300)
plt.show()
...
# Smoothed Right Bank 2 vs Ground Truth
ax3 = axs[2].twinx()
axs[2].plot(merged_df['LocalDateTime'], merged_df['Smoothed_Right_Bank_2'], color='green', label='Smoothed Right Bank 2')
ax3.plot(merged_df['LocalDateTime'], merged_df['Smoothed_Stage'], color='orange', label='Ground Truth', linestyle='--')
axs[2].set_ylabel('No. of Water Pixels')
ax3.set_ylabel('Water Stage (cm)')
lines, labels = axs[2].get_legend_handles_labels()
lines2, labels2 = ax3.get_legend_handles_labels()
axs[2].legend(lines + lines2, labels + labels2, loc='upper left', bbox_to_anchor=(0, 1), ncol=1)
axs[2].set_title('Smoothed Right Bank 2 vs Ground Truth')

# Setting the x-label only on the last subplot
axs[2].set_xlabel('LocalDateTime')

# Adjust layout to prevent overlap
plt.tight_layout()

plt.savefig("individual_bankVSgt_UWRL.png", dpi=300)
# Show the plot
plt.show()